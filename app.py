...
st.markdown("""
<style>
.hero-section { background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 2rem; border-radius: 12px; text-align: center; color: white; margin-bottom: 2rem;}
.hero-title { font-size: 2rem; font-weight: bold; }
.hero-subtitle { font-size: 1.1rem; opacity: 0.9; }

.chat-popup { position: fixed; bottom: 80px; right: 20px; width: 350px; max-height: 500px; background: #fff; border: 1px solid #ddd; border-radius: 12px; box-shadow: 0px 4px 20px rgba(0,0,0,0.2); display: flex; flex-direction: column; z-index: 999; padding: 1rem; }
.chat-messages { flex: 1; overflow-y: auto; margin-bottom: 0.5rem; }
.user-msg { background: #e3f2fd; padding: 0.5rem; margin: 0.3rem; border-radius: 8px; text-align: right; }
.bot-msg { background: #f3e5f5; padding: 0.5rem; margin: 0.3rem; border-radius: 8px; text-align: left; }
.chat-toggle { position: fixed; bottom: 20px; right: 20px; background: #764ba2; color: white; border-radius: 50%; padding: 15px; cursor: pointer; font-size: 20px; text-align: center; box-shadow: 0px 4px 12px rgba(0,0,0,0.3); z-index: 1000; }
</style>
...
class CareerChatbot:
    ...
    def get_response(self, message, history=None):
        context = ("You are a helpful career counsellor. Answer only career-related questions. \n")
                #    "If asked anything else, politely decline: "
                #    "Sorry, I can not answer the question because my data is limited to career counseling.\n")
        ...
        try:
            ...
            if "429" in str(e):
                return "Sorry, the AI service is currently over quota. Please try again later."
            else:
...
if not st.session_state.user:
    ...
    with tab2:
        ...
        if st.button("Sign Up"):
            if st.session_state.db.create_user(u, p):
        ...
        with st.container():
            ...
            if st.button("Send", key="send_btn"):
                if msg.strip():
                    ...
                    st.session_state.chat_history.append({"message": msg, "response": resp, "is_user": 1})
                    st.session_state.chat_history.append({"message": resp, "response": "", "is_user": 0})
